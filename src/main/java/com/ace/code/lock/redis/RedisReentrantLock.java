...
import com.ace.code.lock.AbstractLock;
import com.ace.code.lock.Lock;
import com.ace.code.util.LockInfo;
 ...
 * <b>Usage Example:</b>
 * <pre>
 *	{@link Jedis} jedis = jedisPool.getResource();
 * 	{@link Lock} lock = new {@link RedisReentrantLock}(jedis, "lockKey", lockExpires);
 * 	if (lock.tryLock()) {
 * 		try {
 * 			// do something
 * 		} finally {
...
public class RedisReentrantLock extends AbstractLock {
	...
	private static final String DEFAULT_LOCK_KEY = "lock.key";
	private static final long DEFAULT_LOCK_EXPIRES = 120L; // UNIX时间，单位为秒
	...
	public RedisReentrantLock(Jedis jedis, String lockKey) {
		this(jedis, lockKey, DEFAULT_LOCK_EXPIRES);
	}
	...
	public boolean tryLock() {
		...
		if (isTimeExpired(currLockInfo.getExpires())) {
			...
			if (oldLockInfo != null && isTimeExpired(oldLockInfo.getExpires())) { // 双重isTimeExpired保证多个线程同时进入if语句块中，只有一个getSet方法得到锁
				locked = true;
				return true;
			}
//			jedis.set(lockKey, oldLockInfo.toString()); // 没获取到锁的线程，还原lockKey的值（三个以上的线程同时进入if语句块中交替执行，可能还原的值不正确）
		} else {
	...
	protected void unlock0() {
		...
		if (currLockInfo == null || isTimeExpired(currLockInfo.getExpires())) {
			return;
		}
		...
		if (isHeldByCurrentThread(currLockInfo)) {
			if (currLockInfo.getCount() == 1) {
				jedis.del(lockKey);
				logger.debug("{} unlock(del), lockInfo: null", Thread.currentThread().getId());
			} else {
				currLockInfo.decCount(); // 持有锁计数减1
				String json = currLockInfo.toString();
				jedis.set(lockKey, json);
				logger.debug("{} unlock(dec), lockInfo: {}", Thread.currentThread().getId(), json);
			}
		} else {
			// 正常情况下，任何资源锁的key都会被del，只有当程序异常停止时才会存留，直至被下一个调用覆盖掉（多个线程同时进入覆盖逻辑代码可能会引发下面的警告信息）
			logger.warn(String.format("current thread[%s] does not holds the lock for key[{}]", Thread.currentThread().getId()), lockKey);
		}
	...
	private long serverTimeMillis() {
		return Long.parseLong(jedis.time().get(0)); // time命令返回值：1.UNIX时间以秒为单位 2.微秒
	}
